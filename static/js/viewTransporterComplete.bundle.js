...
/* WEBPACK VAR INJECTION */(function(app, $) {var jszip = __webpack_require__(20);
var exportObj = {
...
/* WEBPACK VAR INJECTION */(function(app, $, btExtend, Vue, commonMsg) {/**
 * Created by MENGJUN on 2017/3/8.
 */
function format(row) {
    // `d` is the original data object for the row
    var str = '<table class="table table-bordered sub-table">' + '<thead>' + '<td>' + vm.message.viewTransporterComplete.table.shipperBidNo + '</td>' + '<td>' + vm.message.viewTransporterComplete.table.shipper + '</td>' + '<td>' + vm.message.viewTransporterComplete.table.jobId + '</td>' + '<td>' + vm.message.viewTransporterComplete.table.postalCodeOrigin + '</td>' + '<td>' + vm.message.viewTransporterComplete.table.postalCodeDestination + '</td>' + '<td>' + vm.message.viewTransporterComplete.table.distance + '</td>' + '<td>' + vm.message.viewTransporterComplete.table.weight + '</td>' + '<td>' + vm.message.viewTransporterComplete.table.volume + '</td>' + '<td>' + vm.message.viewTransporterComplete.table.status + '</td>' + '</thead>' + '<tbody>';

    if (row && row.jobList) {
        for (var i = 0; i < row.jobList.length; i++) {
            var job = row.jobList[i];
            job.companyName = job.companyName ? job.companyName : '';
            str += '<tr>' + '<td>' + job.shipperBidNo + '</td>' + '<td>' + job.companyName + '</td>' + '<td>' + job.id + '</td>' + '<td>' + job.pickupAddress + '</td>' + '<td>' + job.dropoffAddress + '</td>' + '<td>' + job.distance + '</td>' + '<td>' + job.weight + '</td>' + '<td>' + job.volume + '</td>' + '<td>' + app.appOption.displayStatus(job.status) + '</td>';
            str += '</tr>';
        }
    } else {
        ...
        buttons: [{
            ...
            customizeData: function (data) {
                btExtend.exportObj.customerExportData(data, {
                    header: [vm.message.viewTransporterComplete.table.companyName, vm.message.viewTransporterComplete.table.transporterBidNo, vm.message.viewTransporterComplete.table.auctionNo, vm.message.viewTransporterComplete.table.vehicleNo, vm.message.viewTransporterComplete.table.finalCost, vm.message.viewTransporterComplete.table.status],
                    subHeader: [vm.message.viewTransporterComplete.table.shipperBidNo, vm.message.viewTransporterComplete.table.shipper, vm.message.viewTransporterComplete.table.jobId, vm.message.viewTransporterComplete.table.postalCodeOrigin, vm.message.viewTransporterComplete.table.postalCodeDestination, vm.message.viewTransporterComplete.table.distance, vm.message.viewTransporterComplete.table.weight, vm.message.viewTransporterComplete.table.volume, vm.message.viewTransporterComplete.table.status],
                    fieldNames: ["companyName", "bidNo", "auctionNo", "vehicleName", "finalCost", "tripStatus"],
                    subFieldNames: ["shipperBidNo", "companyName", "id", "pickupAddress", "dropoffAddress", "distance", "weight", "volume", "status"],
                    subRowList: "jobList",
                    ajaxParams: {
...
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(4), __webpack_require__(0), __webpack_require__(19), __webpack_require__(1), __webpack_require__(2)))
