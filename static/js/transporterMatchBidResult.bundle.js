...
function format(row) {
    ...
    var str = '<table class="table table-bordered">' + '<thead>' + '<td>' + bmrVue.message.transporterMatchBidResult.table.jobId + '</td>' + '<td>' + bmrVue.message.transporterMatchBidResult.table.postalCodeOrigin + '</td>' + '<td>' + bmrVue.message.transporterMatchBidResult.table.postalCodeDestination + '</td>' + '<td>' + bmrVue.message.transporterMatchBidResult.table.status + '</td>' + '<td>' + bmrVue.message.table.function + '</td>' + '</thead>' + '<tbody>';

    for (var i = 0; i < row.jobList.length; i++) {
        var job = row.jobList[i];
        str += '<tr>' + '<td><a onclick="bmr.viewJob(\'' + row.transporterBidId + '\',\'' + job.id + '\')">' + job.id + '</a></td>' + '<td>' + job.pickupAddress + '</td>' + '<td>' + job.dropoffAddress + '</td>' + '<td>' + app.appOption.displayStatus(job.status) + '</td>';
...
var bmr = {
    ...
    viewDriver: function (driverId) {
        ...
        app.appOption.ajax.request(app.appOption.ajax.url, app.appOption.ajax.type, app.appOption.ajax.data, function (result) {
            if (result) {
                ...
                var attachFiles = result.attachmentFileDtos,
                    imageArr = [];
                if (attachFiles) {
    ...
    setDefaultStatus: function () {
        $("input[name='status'].minimal").each(function (index) {
            if ($(this).val() == "match_success" || $(this).val() == "accept_job" || $(this).val() == "shipper_paid" || $(this).val() == "wait_payment" || $(this).val() == "complete") {
                $(this).iCheck("check");
            }
        });
        bmrTable.draw();
...
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(4), __webpack_require__(0), __webpack_require__(1), __webpack_require__(2)))
