    ...
    avTable = $("#av-table").dataTable($.fn.extend(true, {}, app.appOption.datatables.defaultOptions, {
        ...
        ajax: function (data, callback, settings) {
            ...
            app.appOption.ajax.request(app.appOption.ajax.url, app.appOption.ajax.type, app.appOption.ajax.data, function (result) {
                ...
                $("#view1").empty();
                $("#view1").append(avVue.message.transporterCreateBidTitleInfo.replace("{from}", app.appOption.getQueryParam("from")).replace("{end}", app.appOption.getQueryParam("end")).replace("{vehicleName}", app.appOption.getQueryParam("vehicleName")));
        ...
        columns: [{
            ...
            searchable: false,
            render: function (data, type, row, meta) {
                return '<button id="edit-' + meta.row + '" onclick="av.update(\'edit-' + meta.row + '\',\'' + meta.row + '\');" class="btn btn-app-primary">' + avVue.message.button.edit + '</button>' + '<button onclick="av.delete(' + row.id + ');" class="btn btn-app-danger">' + avVue.message.button.delete + '</button>';
            }
        }]
    })).api();
...
var av = {
    ...
    triggerTimeValidate: function (formObj) {
        ...
        if (formObj.find("select[name='toTime']").val()) av.formValidate(formObj).element(formObj.find("select[name='toTime']"));
    },
    create: function () {
        this.editAvInfo = {};
        this.editRowIndex = -1;
...
jQuery.validator.addMethod("crossTime", function (value, element, param) {
    ...
    var arr = avTable.rows().data(),
        valueFormat = app.timeFormat(value),
        otherValueFormat = app.timeFormat(otherValue),
        viewFrom = app.timeFormat(av.editAvInfo.fromTime),
        viewEnd = app.timeFormat(av.editAvInfo.toTime);
    for (var index in arr) {
        if (arr[index].dayOfWeek === avVue.avInfo.dayOfWeek) {
            var tempFrom = app.timeFormat(arr[index].fromTimeString),
                tempEnd = app.timeFormat(arr[index].toTimeString);
