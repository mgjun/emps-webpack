...
$(document).ready(function () {
    vehicleTable = $("#vehicle-table").dataTable($.fn.extend(true, {}, app.appOption.datatables.defaultOptions, {
        ...
        "columns": [{
                ...
                for (var i = 0; i < avArr.length; i++) {
                    ...
                    for (var j = 0; j < dayOfWeekArr.length; j++) {
                        if (!app.isEmpty(dayOfWeekArr[j])) {
                            resultStr += '<div>' + '<div class="col-sm-4">' + dayOfWeekArr[j] + '</div>' + '<div class="col-sm-6">[' + app.timeFormat(avArr[i].fromTime) + ' - ' + app.timeFormat(avArr[i].toTime) + ']</div>' + '</div>';
                        }
                    }
                }
                return resultStr;
            ...
            "searchable": false,
            "render": function (data, type, row, meta) {
                return '<button onclick="vehicle.update(' + row.id + ')" class="btn btn-sm btn-app-primary tag-show-edit">' + veEditVue.message.button.edit + '</button>' + '<button onclick="vehicle.delete(' + row.id + ')" class="btn btn-sm btn-app-danger tag-show-delete">' + veEditVue.message.button.delete + '</button>';
            }
        }]
    })).api();
