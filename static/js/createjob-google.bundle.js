...
/* WEBPACK VAR INJECTION */(function($) {var apiLink = __webpack_require__(5);
...
var alertify = __webpack_require__(9);
...
var appOption = {
    ...
    fileInput: {
        ...
        validate: {
            valid: function () {
                ...
                if (_self.wrap_op && _self.wrap_op.minFileCount && appOption.fileInput.fileCounts >= _self.wrap_op.minFileCount) {
                    this.success();
                } else {
                    this.error();
                }
            ...
            success: function () {
                var $validFrame = appOption.fileInput.$el.closest(".valid-frame"),
                    $fileInput = $validFrame.find(".file-input");
            ...
            error: function () {
                var $validFrame = appOption.fileInput.$el.closest(".valid-frame"),
                    ...
                    errorMsg = maxCount > minCount ? commonMsg.messages.validate.fileCountBetweenError.replace("{minCount}", minCount).replace("{maxCount}", maxCount) : commonMsg.messages.validate.fileCountLessError.replace("{minCount}", minCount);
                if (!isEmpty($fileInput) && !$validFrame.hasClass("file-error")) {
                    $validFrame.removeClass("file-valid").addClass("file-error").append('<label class="error">' + errorMsg + '</label>').css("display", "block");
    ...
    formOperation: {
        ...
        resetValidation: function (formObjSelector) {
            $(formObjSelector).find(".has-success").removeClass("has-success").closest(formObjSelector).find(".has-error").removeClass("has-error").closest(formObjSelector).find(".valid").removeClass("valid").closest(formObjSelector).find(".error").removeClass("error").closest(formObjSelector).find("label.error").remove();
    ...
    displayStatus: function (status) {
        switch (status) {
            ...
            case "revoke":
                return "Revoked";break;
            case "match_success":
                return "Match Found";break;
            case "match_unsuccess":
                return "No Match Found";break;
            case "accept_job":
                return "Job Accepted";break;
            case "wait_payment":
                return "Pending Payment";break;
            case "shipper_paid":
                return "Shipper Paid";break;
            case "complete":
                return "Completed";break;
            case "shipper_reject":
                return "Shipper Rejected";break;
            case "transporter_reject":
                return "Transporter Rejected";break;
            case "shipper_refund":
                return "Shipper Refunded";break;
            case "refund":
                return "Refunded";break;
            case "refund_apply":
                return "Refund Applied";break;
            case "platform_paid":
                return "Platform Paid";break;
            default:
                return "";
        }
    },
...
(function () {
    var TimeWindow = function ($el, options) {
        this.$el = $el;
        this.defaults = {
            elements: ["00:00", "00:30", "01:00", "01:30", "02:00", "02:30", "03:00", "03:30", "04:00", "04:30", "05:00", "05:30", "06:00", "06:30", "07:00", "07:30", "08:00", "08:30", "09:00", "09:30", "10:00", "10:30", "11:00", "11:30", "12:00", "12:30", "13:00", "13:30", "14:00", "14:30", "15:00", "15:30", "16:00", "16:30", "17:00", "17:30", "18:00", "18:30", "19:00", "19:30", "20:00", "20:30", "21:00", "21:30", "22:00", "22:30", "23:00", "23:30"],
            deleteElements: 0,
            defaultValue: "00:00"
        };
        isEmpty(options) ? this.options = this.defaults : this.options = $.extend({}, this.defaults, options);
        var displayElements = this.options.elements,
            delElCount = this.options.deleteElements;
...
$.validator.prototype.elements = function () {
    ...
    .not(":submit, :reset, :image").not(this.settings.ignore).filter(function () {
 ...
 */
(function ($) {
  function noop() {}function genericCallback(data) {
    lastValue = [data];
    ...
    return (/\?/.test(url) ? "&" : "?"
      ...
      opera = win.opera,
      oldIE = !!$("<div>").html("<!--[if IE]><i><![endif]-->").find("i").length;function jsonp(xOptions) {
    xOptions = $.extend({}, xOptionsDefaults, xOptions);var successCallback = xOptions.success,
        ...
        script,
        scriptAfter,
        timeoutTimer;Deferred && Deferred(function (defer) {
      defer.done(successCallback).fail(errorCallback);successCallback = defer.resolve;errorCallback = defer.reject;
    }).promise(xOptions);xOptions.abort = function () {
      !done++ && cleanUp();
    };if (callIfDefined(xOptions.beforeSend, xOptions, [xOptions]) === !1 || done) {
      return xOptions;
    }url = url || STR_EMPTY;data = data ? typeof data == "string" ? data : $.param(data, xOptions.traditional) : STR_EMPTY;url += data ? qMarkOrAmp(url) + data : STR_EMPTY;callbackParameter && (url += qMarkOrAmp(url) + encodeURIComponent(callbackParameter) + "=?");!cacheFlag && !pageCacheFlag && (url += qMarkOrAmp(url) + "_" + new Date().getTime() + "=");url = url.replace(/=\?(&|$)/, "=" + successCallbackName + "$1");function notifySuccess(json) {
      if (!done++) {
        cleanUp();pageCacheFlag && (pageCache[url] = { s: [json] });dataFilter && (json = dataFilter.apply(xOptions, [json]));callIfDefined(successCallback, xOptions, [json, STR_SUCCESS, xOptions]);callIfDefined(completeCallback, xOptions, [xOptions, STR_SUCCESS]);
      }
    }function notifyError(type) {
      if (!done++) {
        cleanUp();pageCacheFlag && type != STR_TIMEOUT && (pageCache[url] = type);callIfDefined(errorCallback, xOptions, [xOptions, type]);callIfDefined(completeCallback, xOptions, [xOptions, type]);
      }
    }if (pageCacheFlag && (pageCached = pageCache[url])) {
      pageCached.s ? notifySuccess(pageCached.s[0]) : notifyError(pageCached);
    } else {
      win[successCallbackName] = genericCallback;script = $(STR_SCRIPT_TAG)[0];script.id = STR_JQUERY_JSONP + count++;if (charset) {
        script[STR_CHARSET] = charset;
      }opera && opera.version() < 11.60 ? (scriptAfter = $(STR_SCRIPT_TAG)[0]).text = "document.getElementById('" + script.id + "')." + STR_ON_ERROR + "()" : script[STR_ASYNC] = STR_ASYNC;if (oldIE) {
        script.htmlFor = script.id;script.event = STR_ON_CLICK;
      }script[STR_ON_LOAD] = script[STR_ON_ERROR] = script[STR_ON_READY_STATE_CHANGE] = function (result) {
        if (!script[STR_READY_STATE] || !/i/.test(script[STR_READY_STATE])) {
          try {
            script[STR_ON_CLICK] && script[STR_ON_CLICK]();
          } catch (_) {}result = lastValue;lastValue = 0;result ? notifySuccess(result[0]) : notifyError(STR_ERROR);
        }
      };script.src = url;cleanUp = function (i) {
        timeoutTimer && clearTimeout(timeoutTimer);script[STR_ON_READY_STATE_CHANGE] = script[STR_ON_LOAD] = script[STR_ON_ERROR] = null;head[STR_REMOVE_CHILD](script);scriptAfter && head[STR_REMOVE_CHILD](scriptAfter);
      };head[STR_INSERT_BEFORE](script, firstChild = head.firstChild);scriptAfter && head[STR_INSERT_BEFORE](scriptAfter, firstChild);timeoutTimer = timeout > 0 && setTimeout(function () {
 ...
 */
(function (global, factory) {
   true ? module.exports = factory() : typeof define === 'function' && define.amd ? define(factory) : global.VueI18n = factory();
})(this, function () {
  'use strict';
...
/* WEBPACK VAR INJECTION */(function($, app, Vue, commonMsg, jQuery) {/**
...
$(document).ready(function () {
    ...
    $("#additional-services").bind("click", function () {
        $("#additional-popover").css("top", $(this).offset().top + $(this).outerHeight()).css("left", $(this).offset().left);
    ...
    $(".form_datetime").datepicker({
        ...
        var value = $(this).val(),
            ...
            isCurrentDate = value && value.split("-") && parseInt(value.split("-")[0]) == date.getDate() && parseInt(value.split("-")[1]) == date.getMonth() + 1 && parseInt(value.split("-")[2]) == date.getFullYear();
    ...
    $("#job-create-save").bind("click", function () {
        var $formObj = $("#shipper-createJob-form");
        //validate weight
        if (shipperCreateVue.weightInfo.volumn == 0 || shipperCreateVue.weightInfo.weight == 0) {
...
var shipperCreate = {
    ...
    getLocation: function (zoneId) {
        //get location info of the zone
    ...
    edit: {
        ...
        setVehicleType: function () {
            shipperCreateVue.vehicleTypeInfo = Object.assign({}, shipperCreateVue.vehicleTypeInfo, { type: shipperCreateVue.updateJobInfo.vehicleTypeName,
            ...
            for (var index in shipperCreateVue.initAllPackages) {
                for (var temp in shipperCreateVue.selectedPackages) {
                    if (shipperCreateVue.selectedPackages[temp].packageId == shipperCreateVue.initAllPackages[index].id) {
                        shipperCreateVue.initAllPackages[index] = Object.assign({}, shipperCreateVue.initAllPackages[index], { quantity: shipperCreateVue.selectedPackages[temp].quantity });
    ...
    getJobPrice: function (volume, weight, distance, transportDate) {
        //get job price while distance,weight or volume has changed
    ...
    defaultPickUpLocation: function (postalId) {
        for (var index in shipperCreateVue.zones) {
            if (postalId == shipperCreateVue.zones[index].id) {
                ...
                $("input[name='pickupLocation']").closest(".valid-frame").addClass("has-success").removeClass("has-error").find("label.error").remove();
            }
        }
    },
    defaultDropOffLocation: function (postalId) {
        for (var index in shipperCreateVue.zones) {
            if (postalId == shipperCreateVue.zones[index].id) {
                ...
                $("input[name='dropoffLocation']").closest(".valid-frame").addClass("has-success").removeClass("has-error").find("label.error").remove();
            }
        }
    },
...
var shipperCreateVue = new Vue({
    ...
    data: {
        ...
        pickFromInitTimeData: ["00:00", "00:30", "01:00", "01:30", "02:00", "02:30", "03:00", "03:30", "04:00", "04:30", "05:00", "05:30", "06:00", "06:30", "07:00", "07:30", "08:00", "08:30", "09:00", "09:30", "10:00", "10:30", "11:00", "11:30", "12:00", "12:30", "13:00", "13:30", "14:00", "14:30", "15:00", "15:30", "16:00", "16:30", "17:00", "17:30", "18:00", "18:30", "19:00", "19:30", "20:00", "20:30"],
        pickEndInitTimeData: ["00:00", "00:30", "01:00", "01:30", "02:00", "02:30", "03:00", "03:30", "04:00", "04:30", "05:00", "05:30", "06:00", "06:30", "07:00", "07:30", "08:00", "08:30", "09:00", "09:30", "10:00", "10:30", "11:00", "11:30", "12:00", "12:30", "13:00", "13:30", "14:00", "14:30", "15:00", "15:30", "16:00", "16:30", "17:00", "17:30", "18:00", "18:30", "19:00", "19:30", "20:00", "20:30", "21:00", "21:30"],
        dropoffFromInitTimeData: ["00:00", "00:30", "01:00", "01:30", "02:00", "02:30", "03:00", "03:30", "04:00", "04:30", "05:00", "05:30", "06:00", "06:30", "07:00", "07:30", "08:00", "08:30", "09:00", "09:30", "10:00", "10:30", "11:00", "11:30", "12:00", "12:30", "13:00", "13:30", "14:00", "14:30", "15:00", "15:30", "16:00", "16:30", "17:00", "17:30", "18:00", "18:30", "19:00", "19:30", "20:00", "20:30", "21:00", "21:30", "22:00", "22:30"],
    ...
    methods: {
        ...
        showPackageDetail: function (item) {
            ...
            $(".package-popover").removeClass("hide").attr("id", "pop-" + item.id).css("top", $("#package-" + this.currentPackage.id).offset().top + $("#package-" + this.currentPackage.id).outerHeight()).css("left", $("#package-" + this.currentPackage.id).offset().left);
        },
        movePop: function () {
    ...
    computed: {
        countTotalPrice: function () {
            //count total price(additional and job)
...
shipperCreateVue.$watch('distance', function (newValue, oldValue) {
    if (shipperCreateVue.weightInfo.volumn == 0 || shipperCreateVue.weightInfo.weight == 0 || shipperCreateVue.distance == null || app.isEmpty(shipperCreateVue.transportDate)) {
        shipperCreateVue.jobPrice = 0;
        return;
    }
    shipperCreate.getJobPrice(shipperCreateVue.weightInfo.volumn, shipperCreateVue.weightInfo.weight, shipperCreateVue.distance, shipperCreateVue.transportDate);
});
...
function initMap() {
    ...
    $("#pickUpAddress").bind("keyup", function (event) {
        ...
        if (ev >= 48 && ev <= 57 || ev <= 105 && ev >= 96) {
            ...
            } else if (_$this.val().length == 6 && currentKeyUpVal == _$this.val()) {} else {
                shipperCreateVue.pickUpLocation = "";
            }
    ...
    $('#dropOffAddress').bind('keyup', function (event) {
        ...
        if (ev >= 48 && ev <= 57 || ev <= 105 && ev >= 96) {
            ...
            } else if (_$this.val().length == 6 && currentDropKeyUpVal == _$this.val()) {} else {
                shipperCreateVue.dropOffLocation = "";
            }
        ...
        if (status === google.maps.GeocoderStatus.OK) {
            ...
            if (preciseResults && preciseResults.length > 0) {
                ...
                if (shipperCreate.startLocation && shipperCreate.endLocation) {
                    displayRoute(shipperCreate.startLocation, shipperCreate.endLocation, markerArray, directionsService, directionsRender, infoWindow);
                }
            } else {
...
function geocoderByLatLng(latlng, obj, startOrEnd) {
    var geocode = new google.maps.Geocoder(),
        ...
        $pObj = $("#pickUpAddress"),
        $dObj = $("#dropOffAddress"),
        $plObj = $("input[name='pickupLocation']"),
        $dlObj = $("input[name='dropoffLocation']");
...
function geocoderByAddress(address) {
    ...
    geocode.geocode({ address: address, region: "702" }, function (results, status) {
        if (status = google.maps.GeocoderStatus.OK) {} else {
            app.toastr.error(shipperCreateVue.message.directionFailed.replace("{status}", status));
        }
    });
}
...
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(0), __webpack_require__(4), __webpack_require__(1), __webpack_require__(2), __webpack_require__(0)))
...
/* WEBPACK VAR INJECTION */(function($, app, Vue) {__webpack_require__(36);
var shipperCreateJob = __webpack_require__(21);
        ...
        if (status === google.maps.GeocoderStatus.OK) {
            ...
            if (preciseResults && preciseResults.length > 0) {
                ...
                if (shipperCreateJob.shipperCreate.startLocation && shipperCreateJob.shipperCreate.endLocation) {
                    displayRoute(shipperCreateJob.shipperCreate.startLocation, shipperCreateJob.shipperCreate.endLocation, markerArray, directionsService, directionsRender, infoWindow);
                }
            } else {
                geocoderByLatLng(latlng, obj, startOrEnd);
            }
