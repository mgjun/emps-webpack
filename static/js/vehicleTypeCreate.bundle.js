...
$(document).ready(function () {
    priceTable = $("#price-table").dataTable({
        ...
        "columns": [{
            ...
            "searchable": false,
            "render": function (data, type, row, meta) {
                return '<button id="edit-' + meta.row + '" onclick="price.update(\'edit-' + meta.row + '\')" class="btn btn-sm btn-app-primary price-show-edit">' + priceEditVue.message.button.edit + '</button>' + '<button id="delete-' + meta.row + '" onclick="price.delete(\'delete-' + meta.row + '\')" class="btn btn-sm btn-app-danger price-show-delete">' + priceEditVue.message.button.delete + '</button>';
            }
        }],
        "initComplete": function () {
            var button = $(".dataTable tbody tr td button");
    ...
    findById: function (typeId) {
        ...
        app.appOption.ajax.request(app.appOption.ajax.url, app.appOption.ajax.type, app.appOption.ajax.data, function (result) {
            ...
            if (result.priceList) {
                ...
                if (result.priceList.data && result.priceList.data instanceof Array && result.priceList.data.length > 0) {
                    $("#vehicleType-create-save").attr("disabled", false);
                }
            }
        }, function (error) {
