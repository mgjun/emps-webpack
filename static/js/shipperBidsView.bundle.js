...
var bid = {
    ...
    setDefaultStatus: function () {
        ...
        $("input[name='status'].minimal").each(function (index) {
            if ($(this).val() == "draft" || $(this).val() == "submitted" || $(this).val() == "match_success" || $(this).val() == "wait_payment") {
                $(this).iCheck("check");
            }
    ...
    getPayStatus: function (id) {
        ...
        app.appOption.ajax.requestWithStatus(app.appOption.ajax.url, app.appOption.ajax.type, app.appOption.ajax.data, function (result) {
            if (result.result != "SUCCESS" && result.code == "-77") {
                //waiting pay
                setTimeout(function () {
                    bid.getPayStatus(id);
                }, 1000);
            } else if (result.result != "SUCCESS" && result.code == "-88") {
                //pay failed
                $(".content").loadingOverlay("remove");
                $("#payAlert").modal("hide");
                bidTable.draw();
    ...
    pay: function (id, bidNo) {
        ...
        bid.findSystemSettingByKey("PaymentMethod", function (result) {
            if (result) {
                var typeArr = result.value.split("|"),
                    dataArr = [];
