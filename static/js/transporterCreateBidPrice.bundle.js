    ...
    priceTable = $("#price-table").dataTable($.fn.extend(true, {}, app.appOption.datatables.defaultOptions, {
        ...
        ajax: function (data, callback, settings) {
            ...
            app.appOption.ajax.request(app.appOption.ajax.url, app.appOption.ajax.type, app.appOption.ajax.data, function (result) {
                ...
                $("#view1").empty();
                $("#view1").append(priceEditVue.message.transporterCreateBidTitleInfo.replace("{from}", app.appOption.getQueryParam("from")).replace("{end}", app.appOption.getQueryParam("end")).replace("{vehicleName}", app.appOption.getQueryParam("vehicleName")));
                callback(result);
            }, function (error) {
                console.log(error);
            });
        ...
        columns: [{
            ...
            searchable: false,
            render: function (data, type, row, meta) {
                return '<button id="edit-' + meta.row + '" onclick="price.update(\'edit-' + meta.row + '\');" class="btn btn-app-primary">' + priceEditVue.message.button.edit + '</button>' + '<button onclick="price.delete(' + row.id + ');" class="btn btn-app-danger">' + priceEditVue.message.button.delete + '</button>';
            }
        }]
    })).api();
    $(".select2").select2();
