...
$(document).ready(function () {
    avTable = $("#availability-table").dataTable({
        ...
        "columns": [{
            ...
            "searchable": false,
            "render": function (data, type, row, meta) {
                return '<button id="edit-' + meta.row + '" onclick="vehicle.update(\'edit-' + meta.row + '\',\'' + meta.row + '\')" class="btn btn-sm btn-app-primary price-show-edit">' + avVue.message.button.edit + '</button>' + '<button id="delete-' + meta.row + '" onclick="vehicle.delete(\'delete-' + meta.row + '\')" class="btn btn-sm btn-app-danger price-show-delete">' + avVue.message.button.delete + '</button>';
            }
        }],
        "initComplete": function () {
            var button = $(".dataTable tbody tr td button");
...
var vehicle = {
    ...
    triggerTimeValidate: function (formObj) {
        ...
        if (formObj.find("select[name='toTime']").val()) vehicle.formValidate(formObj).element(formObj.find("select[name='toTime']"));
    },
    create: function () {
        this.editAvInfo = {};
        this.editRowIndex = -1;
...
jQuery.validator.addMethod("crossTime", function (value, element, param) {
    ...
    for (var index in arr) {
        if (arr[index].dayOfWeek === avVue.avInfo.dayOfWeek) {
            var compareFromFormat = app.timeFormat(arr[index].fromTime),
                compareToFormat = app.timeFormat(arr[index].toTime);
