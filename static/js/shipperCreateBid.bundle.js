...
$(document).ready(function () {
    ...
    bidJobTable = $("#job-table").dataTable($.fn.extend(true, {}, app.appOption.datatables.defaultOptions, {
        ...
        "columns": [{
            ...
            "searchable": false,
            "render": function (data, type, row, meta) {
                return '<button type="button" id="btn-' + row.id + '" class="btn btn-app-primary" onclick="bid.showDetail(\'' + row.id + '\')">' + bidVue.message.button.view + '</button>' + '<button type="button" id="btn-' + row.id + '" class="btn btn-app-primary" onclick="bid.edit(\'' + row.id + '\')">' + bidVue.message.button.edit + '</button>' + '<button type="button" id="btn-' + row.id + '" class="btn btn-app-danger" onclick="bid.delete(\'' + row.id + '\')">' + bidVue.message.button.delete + '</button>';
            }
        }],
    ...
    $("#bid-create-save").bind("click", function () {
        ...
        app.appOption.ajax.request(app.appOption.ajax.url, app.appOption.ajax.type, app.appOption.ajax.data, function (result) {
            ...
            app.appOption.alertify._init({
                ...
                cancelFn: function (e) {
                    window.location.href = app.isEmpty(bidVue.bidId) ? "./shipperCreateBid.html?sspi=" + app.appOption.getQueryParam("sspi") : "./shipperCreateBid.html?bidId=" + bidVue.bidId + "&sspi=" + app.appOption.getQueryParam("sspi");
                }
            });
        }, function (error) {
            console.log(error);
